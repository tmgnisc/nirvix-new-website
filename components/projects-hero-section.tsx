...
const countryCount = new Set(projects.map((project) => project.location)).size;
const categoryCount = new Set(projects.map((project) => project.category)).size;

const heroStats = [
  { value: `${projects.length}+`, label: "Live websites" },
  { value: `${countryCount}`, label: "Countries served" },
  { value: `${categoryCount}`, label: "Industries covered" },
];

export function ProjectsHeroSection({ onOpenContact }: { onOpenContact?: () => void }) {
  return (
    <div className="relative overflow-hidden bg-sky-wash pt-28 pb-16 sm:pt-36 sm:pb-24">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-6 text-center sm:px-8">
        ...
        <StackedLines
          as="h1"
          lines={["Our Portfolio of", "Websites and Apps"]}
        ...
        <Reveal delay={0.15}>
          <p className="mx-auto mt-6 max-w-2xl text-sm text-ink-soft sm:text-base lg:text-lg">
            Every site below is live and built by Nirvix Technology — travel agencies, property
            marketplaces, news portals, education platforms, and non-profits across Nepal,
            Australia, and New Zealand. Click any project to see it for yourself.
          </p>
        </Reveal>
